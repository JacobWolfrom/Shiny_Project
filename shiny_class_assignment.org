...
#+begin_src R :results none
  ui <- fluidPage(
    sidebarLayout(
      sidebarPanel(
        selectInput(inputId="x", label="X-var", choices=names(mtcars)),
        selectInput(inputId="y", label="Y-var", choices=names(mtcars))
      ),
  ...
  server <- function(input, output) {
    output$plot_out <- output$plot_out <- renderPlot({
